...
static esp_err_t i2c_write8(i2c_port_t port, uint8_t addr, uint8_t reg, uint8_t val)
{
    uint8_t buf[2] = {reg, val};
    return i2c_master_write_to_device(port, addr, buf, sizeof(buf), pdMS_TO_TICKS(I2C_TIMEOUT_MS));
}

static esp_err_t i2c_read_bytes(i2c_port_t port, uint8_t addr, uint8_t reg, uint8_t *data, size_t len)
{
    return i2c_master_write_read_device(port, addr, &reg, 1, data, len, pdMS_TO_TICKS(I2C_TIMEOUT_MS));
}
...
esp_err_t bno_i2c_setup(i2c_port_t port, gpio_num_t sda, gpio_num_t scl, uint32_t clk_hz)
{
    ...
    e = i2c_driver_install(port, conf.mode, 0, 0, 0);
    if (e != ESP_OK)
        return e;
